...
(function ( $ ) { $.fn.linker = function(options)
{
    ...
    this.node = function (data)
    {
        ...
        node.paths_out = {}; // paths out from this node
        node.paths_in = {}; // paths in to this node

		// remove node
        $('h3 .remove', node.el).click(function (e)
        {
            ...
            $.each(node.paths_out, function (_, arr) {
            ...
            $.each(node.paths_in, function (_, arr) {
                $.each(arr, function (_, p) {
        ...
        node.output = function (id, name)
        {
            ...
            node.paths_out[output.__id] = [];

            output.connect = function (input, withoutEvent)
            {
                var nodeOuts = node.paths_out[this.__id];
                ...
                $(path).on('click', function (e, silence) {
                    ...
                    var out_idx = node.paths_out[output.__id].indexOf(conn);
                    node.paths_out[output.__id].splice(out_idx, 1);
        ...
        {
            $(setting_icon).appendTo($('h3', node.el)).click(function () {
                if(node.onSetting)
                    node.onSetting()
            })
        }
    ...
    .on('mouseup', function (e) {
        ...
        if(drag_node) {
            ...
            if(node.onDragFinish)
                node.onDragFinish(parseInt(drag_node.css('left')), parseInt(drag_node.css('top')));
        }
    ...
    .on("mousemove", function(e) {
        ...
        if (drag_node) {
			...
			if(node.onDrag)
				node.onDrag(parseInt(drag_node.css('left')), parseInt(drag_node.css('top')))
        }

        // path
        if(drag_path) {
            $(drag_path).attr('d', curve(drag_path_pos.left, drag_path_pos.top, e.pageX - 5, e.pageY - 5))
        }
    });
